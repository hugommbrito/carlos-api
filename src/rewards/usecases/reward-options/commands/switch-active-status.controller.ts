...
export class SwitchActiveStatusController {
  constructor(
    private readonly switchActiveStatusUsecase: SwitchActiveStatusUsecase
  ){}
  ...
  async update(
    ...
    return { message: `Opção de Recompensa "${result.name}", ${result.isActive ? 'ATIVADA' : 'DESATIVADA'} com sucesso!`, rewardOption: result }
  }
}