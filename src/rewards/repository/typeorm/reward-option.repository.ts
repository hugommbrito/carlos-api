...
import { RewardOption } from "src/rewards/entity/reward-option";
import { Repository } from "typeorm";
import { RewardOptionDomain } from "src/rewards/domains/reward-option.domain";
...
export class RewardOptionRepository implements RewardOptionRepositoryInterface {
  ...
  async create(rewardOption: RewardOptionDomain): Promise<RewardOptionDomain> {
    try{
      ...
      if (rewardPersistedOrError instanceof Error) throw new InternalServerErrorException({}, {description: 'Erro ao persistir opção de recompensa no DB', cause: 'reward-option.repository-try'})

      return RewardOptionMapper.EntityToDomain(rewardPersistedOrError)
    }catch (error) {
      console.error(error)
      throw new InternalServerErrorException({}, {description: 'Erro ao persistir opção de recompensa no DB', cause: 'reward-option.repository-catch'})
    }
  }
  ...
  async update(id: string, rewardOption: RewardOptionDomain): Promise<RewardOptionDomain> {
    try{
      ...
      if (updatedRewardOptionOrError instanceof Error) throw new InternalServerErrorException({},{description: 'Erro ao permanecer as atualizações da opção de recompensa no DB', cause: 'reward-option.repository-try'})

      return RewardOptionMapper.EntityToDomain(rewardOptionToUpdate)
    }catch (error) {
      console.error(error)
      throw new InternalServerErrorException({}, {description: 'Erro ao atualizar opção de recompensa no DB', cause: 'reward-option.repository-catch'})
    }
  }
  ...
  async remove(id: string): Promise<void> {
    ...
    if(!rewardOptionToRemove) throw new NotFoundException({}, {description: 'Opção de recompensa não encontrada', cause: 'reward-option.repository-remove'})

    this.rewardOptionRepository.softRemove(rewardOptionToRemove)
  }
