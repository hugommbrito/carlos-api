...
import { CourseRepository } from 'src/courses/repository/typeorm/course.repository';
...
export class CourseService {
  ...
  async findAll(): Promise<ICourseDomain[]> {
    ...
    if (!courseDomainList)
      throw new NotFoundException(
        {},
        { description: 'Nenhum Curso encontrado', cause: 'course.service' }
      );
  ...
  async findById(id: number): Promise<ICourseDomain> {
    ...
    if (!courseDomain)
      throw new NotFoundException(
        {},
        { description: 'Curso não encontrado', cause: 'course.service' }
      );
