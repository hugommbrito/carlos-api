...
export class UserDomain {
  ...
  public updateWatchedLectures(whatchedLecture: Lecture): boolean {

    const existingIndex = this.props.watchedLectures.findIndex(lecture => lecture.id === whatchedLecture.id)
    if (existingIndex !== -1) {
      this.props.watchedLectures.splice(existingIndex, 1)

      return false
    } else {
      this.props.watchedLectures.push(whatchedLecture)
